...
// Board path positions (0-51 for main track)
const BOARD_PATH = [
  // Red start area (bottom)
  { row: 14, col: 6 },
  { row: 13, col: 6 },
  { row: 12, col: 6 },
  { row: 11, col: 6 },
...
export default function FixedLudoBoard({
  gameState,
  onPieceClick,
  selectedPiece,
  canMove,
  ...
  const getPieceAtPosition = (row: number, col: number) => {
    const pieces: Array<{ playerIndex: number; pieceIndex: number; color: string; canMove: boolean }> = []
    ...
    gameState.players.forEach((player, playerIndex) => {
      player.pieces.forEach((piecePosition, pieceIndex) => {
        ...
        if (piecePosition === -1) {
          ...
          if (homeArea && homeArea.some((pos) => pos.row === row && pos.col === col)) {
            isAtPosition = true
          }
        }

        // Check home paths (52-56)
        if (piecePosition >= 52 && piecePosition <= 56) {
          ...
          if (
            ...
            homePath[homePathIndex].row === row &&
            homePath[homePathIndex].col === col
          ) {
            isAtPosition = true
          }
        }
        ...
        if (isAtPosition) {
          ...
          pieces.push({ playerIndex, pieceIndex, color: player.color, canMove: canMovePiece })
        }
      })
    })
          ...
          Array.from({ length: 15 }, (_, col) => {
            ...
            return (
              <div
                key={`${row}-${col}`}
                className={`
                  aspect-square relative border border-white/20 rounded-sm flex items-center justify-center text-xs
                  ${isPath ? "bg-white/90" : ""}
                  ${isHome && cellColor ? `bg-${cellColor}-200` : ""}
